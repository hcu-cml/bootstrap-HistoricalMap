...
class LayerSwitcher extends Control {
    ...
    static isBaseGroup(grp) {
        if (grp instanceof LayerGroup) {
            ...
            return lyrs.length && lyrs[0].get('type') === 'base1';
        }
        else if (grp instanceof LayerGroup){
            const lyrs = grp.getLayers().getArray();
            return lyrs.length && lyrs[0].get('type') === 'base2';
        }
        else {
            return false;
        }
    ...
    static ensureTopVisibleBaseLayerShown(map, groupSelectStyle) {
        ...
        LayerSwitcher.forEachRecursive(map, function (lyr, _idx, _arr) {
            if (lyr.get('type') === 'base1' && lyr.getVisible()) {
                lastVisibleBaseLyr = lyr;
            }
            if (lyr.get('type') === 'base2' && lyr.getVisible()) {
                lastVisibleBaseLyr = lyr;
            }
        });
    ...
    static renderLayer_(map, lyr, idx, options, render) {
        ...
        if (lyr instanceof LayerGroup && !lyr.get('combine')) {
            ...
            li.className = 'layer';
            const input = document.createElement('input');
            if (lyr.get('type') === 'base1' || lyr.get('type') === 'base2') {
                input.type = 'radio';
            }else if (lyr.get('type') === 'header'){
            li.className = 'layer-group-title';
            li.textContent = lyr.get('title');
            return li; // don’t add a checkbox
            }
            else {
                input.type = 'checkbox';
            }
